...
{
    public void DrawChannelName(Tab activeTab, bool sendChannelSwitch = false, bool debug = false)
    {
    ...
    public static void DrawIcon(Chunk chunk, IconChunk icon, PayloadHandler? handler)
    {
    ...
    private void DrawChunk(Chunk chunk, bool wrap = true, PayloadHandler? handler = null, float lineWidth = 0f)
    {
        ...
        if (wrap)
        {
            ImGuiUtil.WrapText(content, chunk, handler, InputHandler.DefaultText, lineWidth);
        }
        else
        {
